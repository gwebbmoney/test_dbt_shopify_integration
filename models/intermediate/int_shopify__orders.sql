...
SELECT DISTINCT(oi.id) AS order_id,
    ...
    REGEXP_SUBSTR(note_attributes, '"name"\s*:\s*"referrer"\s*,\s*"order_id"\s*:\s*null\s*,\s*"value"\s*:\s*"([^"]*)"', 1, 1, 'i', 1) AS referrer,
    REGEXP_SUBSTR(note_attributes, '"name"\s*:\s*"pws"\s*,\s*"order_id"\s*:\s*null\s*,\s*"value"\s*:\s*"([^"]*)"', 1, 1, 'i', 1) AS pws,
    REGEXP_SUBSTR(note_attributes, '"name"\s*:\s*"PartyWebAlias"\s*,\s*"order_id"\s*:\s*null\s*,\s*"value"\s*:\s*"([^"]*)"', 1, 1, 'i', 1) AS partywebalias,
    o.note_attributes,
