...
SELECT id AS order_line_id,
    ...
    gift_card
FROM order_lines ol LEFT JOIN {{ ref("redaspen_product_variants") }} p ON ol.sku = p.sku AND ol.product_variant_id = p.shopify_product_variant_id
    LEFT JOIN {{ ref("redaspen_bundle_variants") }} b ON ol.sku = b.sku AND ol.product_variant_id = p.shopify_bundle_variant_id